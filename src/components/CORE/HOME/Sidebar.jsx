  ...
  return (
    <div className='w-3/12 sm:w-2/12  h-full border-r-2'>
      <div className='h-full flex-col justify-center items-start gap-16'>
        <h1 className='text-lg sm:text-2xl font-bold text-black p-2'>Zeiglar Byte</h1>
        <div className='w-full h-[80%] overflow-y-scroll gap-6 text-lg flex flex-col mt-4 overflow-auto overflow-x-hidden p-1'>
          {
            courses.map((course) => {
              return (
                <div key={course._id}
                ...
                >
                  <p 
                    className={`w-full font-normal cursor-pointer flex items-center gap-2 ${hoveredLink === course._id && " bg-yellow-600 text-white "}`}
        ...
        <div className='w-full bg-btn-red text-white text-lg sm:text-2xl p-3 rounded-md m font-semibold text-center hover:cursor-pointer '
          onClick={()=>{
            if(!token){
              navigate('/login')
            } else{
