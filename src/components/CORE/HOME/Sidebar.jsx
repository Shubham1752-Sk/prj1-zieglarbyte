...
const Sidebar = ({ courses, setSelectedCourse, selectedCourse, viewSideBar, setViewSideBar, viewHamBurgerMenuButton, setViewHamBurgerMenuButton }) => {
  ...
  return (
    <>
      <div className={`w-3/12 sm:block sm:w-2/12  h-full sm:border-r-2 ${viewSideBar ? "block absolute w-[98%] bg-white z-20 ": "w-0 relative " } ${viewSideBar && "ease-out w-full origin-right transition-[width] duration-[.5s] z-10 "}`}>
        ...
        <RxCross2 className={`sm:hidden mt-4 ml-2 p-2 text-4xl hover:bg-[#e5e2e2] hover:cursor-pointer ${viewSideBar && !viewHamBurgerMenuButton && "block"}`}  onClick={()=>{setViewSideBar((prev)=>!prev);setViewHamBurgerMenuButton((prev)=>!prev)}} />
          ...
          <div className='p-2 flex justify-center '>
            <div className={`w-full md:w-10/12 lg:w-full bg-btn-red text-white text-sm md:text-lg sm:text-lg p-1 lg:p-3 rounded-md m font-semibold text-center hover:cursor-pointer ${viewSideBar && "-mt-6 py-2 text-xl"}`}
              onClick={() => {
                if (!token) {
                  navigate('/login')
                } else {
